...
extended(ParserStream).extend({
  ...
  __processRows: function(rows, data, cb) {
    ...
    extended.asyncEach(
      ...
      function(row, cb) {
        if (row) {
          self.__handleLine(row, (count = ++self._rowCount), false, function(
            err,
            dataRow
          ) {
            if (err) {
  ...
  validate: function(cb) {
    if (!extended.isFunction(cb)) {
      this.emit(
        'error',
        new TypeError('fast-csv.Parser#validate requires a function')
      );
    }
    if (cb.length === 2) {
      this.__validate = cb;
  ...
  transform: function(cb) {
    if (!extended.isFunction(cb)) {
      this.emit(
        'error',
        new TypeError('fast-csv.Parser#transform requires a function')
      );
    }
    if (cb.length === 2) {
      this.__transform = cb;
